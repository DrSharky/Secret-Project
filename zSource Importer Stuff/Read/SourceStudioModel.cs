...
namespace uSrcTools
{
	...
	public class SourceStudioModel// : MonoBehaviour
	{
		...
		public SourceStudioModel Load(string ModelName)
		{
            ...
            if (!File.Exists(curModelName))
            {
                Debug.Log("No model found at: " + ResourceManager.GetPath(curModelName));
                return null;
            }
            if (!File.Exists(curModelName.Replace(".mdl", ".vvd")))
            {
                Debug.Log("Invalid model, no .vvd file found!");
                return null;
            }
            if (!File.Exists(curModelName.Replace(".mdl", ".dx80.vtx")))
            {
                Debug.Log("Invalid model, no .dx90.vtx file found!");
                return null;
            }
			...
			{
                Debug.Log("Model parsing went wrong!");
				return null;
			}
